...
from .models import Services, CategorysServices
from django.views.generic import ListView, DetailView, TemplateView

class ServiceView(ListView):
    model = Services
    template_name = "active/services/services.html"
    ...
    def setup(self, request, *args, **kwargs):
        ...
        if self.category_slug:
            try:
                self.current_category = CategorysServices.objects.get(slug=self.category_slug)
            except CategorysServices.DoesNotExist:
                self.current_category = None
    ...
    def get_template_names(self):
        ...
        if is_htmx and self.request.GET.get('page'):
            return ["active/services/partials/service_page_content.html"]
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        if self.request.headers.get("HX-Request") and self.request.GET.get('page'):
            return render(self.request, "active/services/partials/service_page_content.html", context)
        return super().render_to_response(context, **response_kwargs)

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)

        context['categorys'] = CategorysServices.objects.all()
...
class ServicePaginationView(ListView):
    model = Services
    template_name = "active/services/partials/service_items.html"  # Только элементы
    context_object_name = "services"
    paginate_by = 6
...
class ServiceDetailView(DetailView):
    model = Services
    template_name = "active/services/service_detail.html"
    context_object_name = "service"


# Create your views here.

class ServiceModerationView(CustomHtmxMixin, View):
    template_name = 'moderation/services/services.html'
